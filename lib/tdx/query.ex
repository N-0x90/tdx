defmodule Tdx.Query do
  ...
  Once a query is prepared with `Tdx.prepare/4`, the
  returned query will have its `ref` field set to a reference.
  When `Tdx.execute/4` is called with the prepared query,
...
defimpl DBConnection.Query, for: Tdx.Query do
  require Tdx.Messages
  ...
  #   case Tdx.Types.encode_params(params, param_types, types) do
  ...
  def decode(_, %Tdx.Result{rows: nil} = res, _opts) do
    res
  end

  def decode(_, %Tdx.Result{rows: rows} = res, opts) do
    %Tdx.Result{res | rows: decode_map(rows, opts)}
  end
...
defimpl String.Chars, for: Postgrex.Query do
  def to_string(%Tdx.Query{statement: statement}) do
