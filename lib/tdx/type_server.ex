defmodule Tdx.TypeServer do
    ...
    """
    @spec fetch(pid) ::
            {:lock, reference, Tdx.Types.state()} | :noproc | :error
    def fetch(server) do
      try do
    ...
    """
    @spec update(pid, reference, [Tdx.TypeInfo.t()]) :: :ok
    ...
    defp associate(%{types: types, lock: ref} = state, type_infos, from) do
      Tdx.Types.associate_type_infos(type_infos, types)
      Process.demonitor(ref, [:flush])
