defmodule Tdx.Protocol do
  @moduledoc false

  alias Tdx.{Types, TypeServer, Query, Cursor, Copy}
  import Tdx.{Messages, BinaryUtils}
  require Logger
  use DBConnection
  ...
  @type state :: %__MODULE__{
          ...
          queries: nil | :ets.tid(),
          mssql: DBConnection.status() | {DBConnection.status(), reference},
  ...
  @spec connect(Keyword.t()) ::
          {:ok, state}
          | {:error, Tdx.Error.t() | %DBConnection.ConnectionError{}}
  def connect(opts) do
  ...
  defp host_and_port(opts) do
    ...
    #       :error ->
            case Keyword.fetch(opts, :hostname) do
              ...
              :error ->
                raise ArgumentError, "expected :hostname to be given"
            end
    #     end
    # end
  end
  ...
  defp handshake(%{sock: {:gen_tcp, sock}, timeout: timeout} = s, status) do
    ...
    case do_handshake(%{s | peer: peer}, status) do
      {:ok, %{parameters: parameters} = s} ->
        ...
        ref = Tdx.Parameters.insert(parameters)
        {:ok, %{s | parameters: ref}}
  ...
  defp auth_recv(s, status, buffer) do
    case msg_recv(s, :infinity, buffer) do
      ...
      {:ok, msg_error(fields: fields), buffer} ->
        disconnect(s, Tdx.Error.exception(mssql: fields), buffer)
  ...
  defp disconnect(%{connection_id: connection_id} = s, %Tdx.Error{} = err, buffer) do
