defmodule Tdx do
  ...
  This module handles the connection to SQL Server, providing support
  for queries, transactions and more.
  """
  ...
  """
  @spec start_link([start_option]) :: {:ok, pid} | {:error, Tdx.Error.t() | term}
  def start_link(opts) do
    ensure_deps_started!(opts)
    opts = Tdx.Utils.default_opts(opts)
    DBConnection.start_link(Tdx.Protocol, opts)
  end
  ...
  ## Examples
      ...
      Tdx.query(conn, "CREATE TABLE posts (id serial, title text)", [])

      Tdx.query(conn, "INSERT INTO posts (title) VALUES ('my title')", [])

      Tdx.query(conn, "SELECT title FROM posts", [])

      Tdx.query(conn, "SELECT id FROM posts WHERE title like $1", ["%my%"])

      Tdx.query(conn, "COPY posts TO STDOUT", [])
  """
  @spec query(conn, iodata, list, [execute_option]) ::
          {:ok, Tdx.Result.t()} | {:error, Exception.t()}
  def query(conn, statement, params, opts \\ []) do
    if name = Keyword.get(opts, :cache_statement) do
      ...
      case DBConnection.prepare_execute(conn, query, params, opts) do
        ...
        {:error, %Tdx.Error{mssql: %{code: :feature_not_supported}}} = error ->
  ...
  """
  @spec query!(conn, iodata, list, [execute_option]) :: Tdx.Result.t()
  def query!(conn, statement, params, opts \\ []) do
  ...
  @spec prepare(conn, iodata, iodata, [option]) ::
          {:ok, Tdx.Query.t()} | {:error, Exception.t()}
  ...
  """
  @spec prepare!(conn, iodata, iodata, [option]) :: Tdx.Query.t()
  def prepare!(conn, name, statement, opts \\ []) do
    opts = Keyword.put(opts, :postgrex_prepare, true)
  ...
  @spec prepare_execute(conn, iodata, iodata, list, [execute_option]) ::
          {:ok, Tdx.Query.t(), Tdx.Result.t()} | {:error, Tdx.Error.t()}
  ...
  """
  @spec execute(conn, Tdx.Query.t(), list, [execute_option]) ::
          {:ok, Tdx.Query.t(), Tdx.Result.t()} | {:error, Tdx.Error.t()}
  ...
  """
  @spec close(conn, Tdx.Query.t(), [option]) :: :ok | {:error, Exception.t()}
  ...
  """
  @spec close!(conn, Tdx.Query.t(), [option]) :: :ok
  def close!(conn, query, opts \\ []) do
  ...
  def parameters(conn, opts \\ []) do
    DBConnection.execute!(conn, %Tdx.Parameters{}, nil, opts)
  end
  ...
  def child_spec(opts) do
    ensure_deps_started!(opts)
    opts = Tdx.Utils.default_opts(opts)
    DBConnection.child_spec(Tdx.Protocol, opts)
  end
  ...
  """
  @spec stream(DBConnection.t(), iodata | Tdx.Query.t(), list, [option]) ::
          Tdx.Stream.t()
  ...
  def stream(%DBConnection{} = conn, query, params, options \\ []) do
    ...
    # %Tdx.Stream{conn: conn, query: query, params: params, options: options}
  end
