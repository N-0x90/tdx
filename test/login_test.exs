defmodule LoginTest do
  ...
  setup do
    {:ok, [options: [database: "tdx_test", backoff_type: :stop, max_restarts: 0]]}
  end

  test "login cleartext password", context do
    Process.flag(:trap_exit, true)
    opts = [username: "tdx_cleartext_pw", password: "tdx_cleartext_pw"]
    assert {:ok, pid} = T.start_link(opts ++ context[:options])
    assert {:ok, %Tdx.Result{}} = T.query(pid, "SELECT 123", [])
  end
